...
* **tokenMax A, B (increase\_liquidity)** - The maximum amount of token X to add to the position. Note the value here is shifted by the decimal places of the token.
* **tokenMin A, B (decrease\_liquidity)** - The minimum amount of token X to withdraw from the position. Note the value here is shifted by the decimal places of the token.
...
* `TokenMaxExceeded` (0x1781) - The required token to perform this operation exceeds the user defined amount in increase\_liquidity.
* `TokenMinSubceeded` (0x1782) - The required token to perform this operation subceeds the user defined amount in decrease\_liquidity.
